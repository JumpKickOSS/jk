...
// The production package graph, shared by G66 (ownership) and G67 (the cycle band). This gate
// cannot read buildSrc, so it carries a twin of `PackageGraph`'s scope by hand — both sides name the
// four tiers and the one excluded tree so the two can be diffed. Two guards deriving "the
// production packages" separately is two things to drift.
val packageTiers = listOf("shared/", "server/", "clients/", "plugins/")
val packageScanned: List<Path> = mainJava.filter { f -> packageTiers.any { rel(f).startsWith(it) } }
val packageDeclPattern = Regex("""(?m)^\s*package\s+([A-Za-z_][\w.]*)\s*;""")
val firstPartyImportPattern = Regex("""(?m)^\s*import\s+(?:static\s+)?(cc\.jumpkick\.[\w.]*?)\s*;""")

/** module -> package -> the sources declaring it, over the four tiers. */
val packageSources: Map<String, Map<String, List<Path>>> by lazy {
    val out = sortedMapOf<String, MutableMap<String, MutableList<Path>>>()
    packageScanned.forEach { f ->
        val here = rel(f)
        val marker = here.indexOf("/src/main/java/")
        if (marker < 0) return@forEach
        val pkg = packageDeclPattern.find(text(f))?.groupValues?.get(1) ?: return@forEach
        out.getOrPut(here.substring(0, marker)) { sortedMapOf() }.getOrPut(pkg) { mutableListOf() }.add(f)
    }
    out
}

/** Non-trivial strongly-connected components of [edges] over [nodes], largest first (Tarjan). */
fun packageComponents(nodes: Set<String>, edges: Map<String, Set<String>>): List<List<String>> {
    ...
    fun strong(v: String) {
        ...
        (edges[v] ?: emptySet()).sorted().forEach { w ->
            if (w !in idx) { strong(w); low[v] = minOf(low.getValue(v), low.getValue(w)) }
            else if (w in onStack) low[v] = minOf(low.getValue(v), idx.getValue(w))
        }
        if (low.getValue(v) == idx.getValue(v)) {
            val comp = mutableListOf<String>()
            while (true) {
                val w = stack.removeLast(); onStack.remove(w); comp.add(w)
                if (w == v) break
            }
            if (comp.size > 1) found.add(comp.sorted())
        }
    }
    nodes.sorted().forEach { if (it !in idx) strong(it) }
    return found.sortedWith(compareByDescending<List<String>> { it.size }.thenBy { it.first() })
}

/**
 * module -> package -> the packages it imports, within the same module.
 *
 * <p>An import resolves to the longest declared-package prefix WITHIN the module, which is what
 * lands a static or nested-type import on the package that declares it; imports leaving the module
 * are dropped, since a cross-module edge cannot close a cycle the module graph already forbids.
 *
 * <p>Built once and shared by the cycle walk and the edge count. The count is DISTINCT
 * package-to-package edges per module, matching `PackageGraph.edgeCount` on the Gradle side — the
 * same edge carried by twenty files is one edge, and summing per file gave 1,028 against Gradle's
 * 195 for one tree.
 */
val packageEdges: Map<String, Map<String, Set<String>>> by lazy {
    val out = sortedMapOf<String, Map<String, Set<String>>>()
    packageSources.forEach { (module, own) ->
        val declared = own.keys.toSortedSet()
        val edges = sortedMapOf<String, MutableSet<String>>()
        own.forEach { (pkg, files) ->
            files.forEach { f ->
                firstPartyImportPattern.findAll(text(f)).forEach { m ->
                    val imported = m.groupValues[1]
                    val target =
                        declared.filter { imported == it || imported.startsWith("$it.") }.maxByOrNull { it.length }
                    if (target != null && target != pkg) edges.getOrPut(pkg) { sortedSetOf() }.add(target)
                }
            }
        }
        out[module] = edges
...
/** Packages of [module] inside a non-trivial strongly-connected component of its package graph. */
fun packageCycles(module: String): List<List<String>> {
    val own = packageSources[module] ?: return emptyList()
    return packageComponents(own.keys.toSortedSet(), packageEdges[module] ?: emptyMap())
}

fun packageEdgeCount(): Int = packageEdges.values.sumOf { module -> module.values.sumOf { it.size } }

guard("G66", "checkPackageModuleOwnership") {
    // One module owns each production package; `package-owners.txt` is the map of legal exceptions.
    // A split package is latent rather than active harm — package-private reach works across a jar
    // boundary on a flat classpath and stops working under JPMS — which is why nine of the eleven
    // current rows grew to 30, 25, 27 and 20 files without breaking anything.
    val ownersByPackage = sortedMapOf<String, MutableSet<String>>()
    packageSources.forEach { (module, byPkg) ->
        byPkg.keys.forEach { pkg -> ownersByPackage.getOrPut(pkg) { sortedSetOf() }.add(module) }
    }
    val roots = packageSources.keys
    val corpus = "${packageScanned.size} main sources, ${ownersByPackage.size} packages, ${roots.size} module roots"
    ...
    if (packageScanned.size < 1_200 || ownersByPackage.size < 80 || roots.size < 25) {
...
guard("G67", "checkPackageCycleBand") {
    // Per-module band, both directions. Over budget is a regression; under budget means the
    // baseline is loose and the commit that improved the structure must bank it. The message names
    // the component's PACKAGES, never its cycles: a 15-node component has thousands of elementary
    // cycles, and none of them says which edge to cut.
    val roots = packageSources.keys
    val packages = packageSources.values.flatMap { it.keys }.toSortedSet()
    val edgeCount = packageEdgeCount()
    val corpus = "${packageScanned.size} main sources, ${packages.size} packages, " +
        "${roots.size} module roots, $edgeCount package edges"
    // Floors, measured at landing: 1,432 / 95 / 29 / 195. An empty edge set reports every module
    // acyclic, which is the one failure this guard must never report as success.
    if (packageScanned.size < 1_200 || packages.size < 80 || roots.size < 25 || edgeCount < 100) {
        error("Scanned $corpus (measured at landing: 1,432 main sources, 95 packages, 29 module" +
            " roots, 195 package edges), so this guard cannot report green — the scope or the" +
            " import resolver drifted.")
    }
    val baseline = sortedMapOf<String, Int>()
    val malformed = mutableListOf<String>()
    Files.readAllLines(at("cycle-baseline.txt")).forEachIndexed { i, raw ->
        val line = raw.substringBefore('#').trim()
        if (line.isEmpty()) return@forEachIndexed
        val parts = line.split(Regex("\\s+"))
        ...
        if (parts.size != 2 || count == null) {
            ...
            baseline[parts[0]] = count
        }
    }
    if (malformed.isNotEmpty()) error("cycle-baseline.txt is malformed:\n" + bullets(malformed))
    val faults = mutableListOf<String>()
    val tightened = mutableListOf<String>()
    roots.forEach { module ->
        val found = packageCycles(module)
        val count = found.sumOf { it.size }
        val budget = baseline[module] ?: 0   // absent means 0, as walk-baseline.txt already does
        if (count == budget) return@forEach
        val shape = found.joinToString("; ") { c ->
            "(${c.size}) " + c.joinToString(", ") { it.removePrefix("cc.jumpkick.") }
        }
        if (count > budget) {
            faults.add("$module: $count packages in a cycle, banded at $budget\n      component" +
                (if (found.size == 1) "" else "s") + ": $shape")
        } else {
    ...
    if (faults.isNotEmpty()) {
        error("A module's package-cycle count may not grow ($corpus):\n" + bullets(faults.sorted()) +
            "\n  Cut an edge, or widen the module's cycle-baseline.txt line and say why.")
    }
    if (tightened.isNotEmpty()) {
        error("cycle-baseline.txt is loose — a module improved and the commit must bank it." +
            " Paste these lines ($corpus):\n" + bullets(tightened.sorted()))
    }
}
...
guard("G53", "checkNullMarkedApiPackages") {
    ...
    val packages = sources.asSequence()
        ...
        .toSortedSet()
    if (packages.size != 47) {
        error("Found ${packages.size} enforced production packages; measured against 47. The source"
