...
// Module-owned guards.
//
// These read one module's sources, which under Gradle is why they sat in that module's
// build.gradle.kts rather than in buildSrc. Here they share the gate's one fork and one lexer:
// a `.jk/` of their own would cost a second kotlinc start-up per module and a second copy of
// every primitive above, which is the duplication the rest of this file exists to prevent. The
// failure still names the module and the file, so the fix stays local.
// ---------------------------------------------------------------------------

val cliMainJava: List<Path> = filesUnder(at("clients/cli/src/main/java"), ".java")
val cliTestJava: List<Path> = filesUnder(at("clients/cli/src/test/java"), ".java")
val engineMain: List<Path> = filesUnder(at("server/engine/src/main/java"), ".java")
val engineTest: List<Path> = filesUnder(at("server/engine/src/test/java"), ".java")

// ---------------------------------------------------------------------------
// checkCliNoParseTypes — native reachability.
//
// The native image's reachability graph must not pull in the TOML parser or the plugin schema.
// These are types the slim client has no business naming; `TomlScan` and `MinimalToml` are
// deliberately absent from the list — they are the line scanner and the scalar codec the image
// is SUPPOSED to reach.
// ---------------------------------------------------------------------------
val cliBannedTypes = listOf(
    "JkBuildParser", "PluginDescriptor", "PluginTableRegistry", "org.tomlj", "TomlValues",
    "LockFreshness", "LockManifestDigest", "PluginContributions", "Giter8LocalApply",
    "Giter8Apply", "Giter8ShortNames", "Giter8TemplateIndex", "org.stringtemplate", "org.antlr.runtime")

guard("CLI1", "checkCliNoParseTypes") {
    if (cliMainJava.size < 200) {
        error("scanned ${cliMainJava.size} files under clients/cli/src/main/java; measured against 274.")
    }
    val hits = cliMainJava.flatMap { f ->
        val raw = text(f)
        cliBannedTypes.filter { raw.contains(it) }.map { "${rel(f)}: $it" }
    }
    if (hits.isNotEmpty()) {
        error("CLI main must not reference parser / plugin-schema types — every one of them drags"
            + " tomlj or ANTLR into the native image's reachability graph:\n" + bullets(hits))
    }
}

// ---------------------------------------------------------------------------
// checkCliRuntimeClasspath — the slim client links no plugin SPI and no test framework.
//
// Gradle asks the resolved `runtimeClasspath` for this. jk's answer is the manifest, which is
// where the mistake is actually typed: a `jk-plugin-sdk` or a junit/assertj edge in [dependencies]
// puts the jar on the client's runtime, and a `kind = "tests"` edge outside a [test-*] table puts
// test code in the image. The two builds check the same fact from the two places each can see.
// ---------------------------------------------------------------------------
val cliForbiddenRuntimeDeps = setOf(
    "jk-plugin-sdk", "maven-artifact", "plexus-utils", "jline-terminal-ffm",
    "junit-jupiter", "junit-platform-engine", "junit-platform-launcher", "assertj-core")

guard("CLI2", "checkCliRuntimeClasspath") {
    val manifest = at("clients/cli/jk.toml")
    if (!Files.isRegularFile(manifest)) error("clients/cli has no jk.toml, so this guard reads nothing.")
    val hits = mutableListOf<String>()
    var table = ""
    var mainEdges = 0
    Files.readAllLines(manifest).forEach { raw ->
        ...
        if (line.startsWith("[")) { table = line.trim('[', ']'); return@forEach }
        if (!table.endsWith("dependencies")) return@forEach
        // Only the main-tier tables reach the native image; [test-*] and [provided-*] do not.
        if (table.startsWith("test-") || table.startsWith("provided-") || table.startsWith("processor-")) return@forEach
        val name = Regex("""^([\w-]+)\s*[.=]""").find(line)?.groupValues?.get(1) ?: return@forEach
        mainEdges++
        if (name in cliForbiddenRuntimeDeps) hits.add("clients/cli/jk.toml [$table] declares $name")
        if (Regex("""kind\s*=\s*"tests"""").containsMatchIn(line)) {
            hits.add("clients/cli/jk.toml [$table] takes $name's test classes — that is test code on"
                + " the native client's runtime")
        }
    }
    if (mainEdges == 0) {
        error("clients/cli/jk.toml declares no main-tier dependency at all, so this guard verified"
            + " nothing. Fix the scan before trusting a green run.")
    }
    if (hits.isNotEmpty()) {
        error("The native client's runtime must not carry the plugin SPI, a Maven/JLine library or"
            + " any test framework:\n" + bullets(hits)
            + "\n  Everything heavy reaches the engine over the wire. A type the CLI genuinely needs"
            + " belongs on :host or :wire, not on the SPI.")
    }
}

// ---------------------------------------------------------------------------
// G27 — the terminal is handed to a child in exactly one place.
//
// `CliOutput.handOffTerminal(pb)` restores the terminal out of whatever mode jk put it in and
// THEN starts the child on inherited stdio. Those two steps are a pair, and spelling them
// separately is how three handoff sites came to skip the restore — an interactive child's own
// line editing did not work. The pattern is `inheritIO`, not `new ProcessBuilder`: this module
// forks plenty of children that must NOT inherit stdio.
// ---------------------------------------------------------------------------

guard("G27", "checkOneTerminalHandoff") {
    if (cliMainJava.size < 200) {
        error("scanned ${cliMainJava.size} files under clients/cli/src/main/java; measured against 285.")
    }
    val ownerFile = cliMainJava.singleOrNull { it.fileName.toString() == "CliOutput.java" }
        ?: error("CliOutput.java is not under clients/cli/src/main/java any more, so this guard has"
            + " lost the owner it exempts.")
    if (!codeOf(ownerFile).contains("inheritIO()")) {
        error("CliOutput.java no longer calls inheritIO(), so handOffTerminal has stopped being the"
            + " handoff and this guard is exempting a file that does nothing.")
    }
    val hits = cliMainJava.filter { it != ownerFile && codeOf(it).contains("inheritIO(") }.map { rel(it) }
    if (hits.isNotEmpty()) {
        error("Handing this terminal to a child means restoring it out of jk's mode first, and that"
            + " pair has one owner: CliOutput.handOffTerminal(pb). These files inherit stdio"
            + " themselves:\n" + bullets(hits)
...
// ---------------------------------------------------------------------------
// G31 — a test that names the ambient state root declares it throwaway.
//
// The tier's JK_HOME is one root shared by every parallel fork and by every run before this one,
// so `state/` is ambient input. A test that resolves it and then asserts on what it finds is
// asserting against the last run: one suite planted a fixture in the shared `state/aot` and a
// sibling fork's sweep deleted it mid-assertion — three greens and a red from the same bytes.
// The rule is not "never touch the root", it is "name the root, declare the isolation".
// ---------------------------------------------------------------------------

guard("G31", "checkTestRootsDeclared") {
    if (cliTestJava.isEmpty()) error("no tests under clients/cli/src/test/java, so this guard verified nothing.")
    val accessors = listOf("JkDirs.state()", "JkDirs.builds()")
    val hits = cliTestJava.flatMap { f ->
        val raw = text(f)
        if (raw.contains("@IsolatedState")) emptyList()
        else accessors.filter { raw.contains(it) }.map { "${rel(f)}: $it" }
    }
    if (hits.isNotEmpty()) {
        error("The tier's JK_HOME is shared across forks and across runs, so a test that reads the"
            + " ambient state root inherits state instead of establishing it. Annotate the class with"
            + " @IsolatedState:\n" + bullets(hits))
    }
}

// ---------------------------------------------------------------------------
// G32 — a spike-cache test roots its project in a @TempDir.
//
// The persistent spike cache is keyed by action id, which includes the project's absolute path.
// A test that reuses a fixed root replays the stored record and the code under test never
// executes — a green run that proves nothing. A per-method @TempDir qualifies the id afresh.
//
...
guard("G32", "checkSpikeCacheTempDir") {
    val marker = "android-spike-cache"
    // Declared as a path rather than matched by name in a regex: moving or renaming the file fails
    // this guard loudly instead of silently narrowing the exemption to nothing.
    val exemptPath = "server/engine/src/test/java/cc/jumpkick/runtime/NiaScratchTest.java"
    if (!Files.isRegularFile(at(exemptPath))) {
        error("the one exemption, $exemptPath, is gone. Delete the exemption in the same change, or"
            + " restore the file.")
    }
    val hits = engineTest.filter { text(it).contains(marker) }
    if (hits.size < 10) {
        error("only ${hits.size} file(s) under server/engine/src/test/java name \"$marker\""
            + " (18 when this guard was measured). That is a blind scan — a renamed marker or a moved"
            + " tree — not a clean population.")
    }
    val bad = hits.filter { rel(it) != exemptPath && !text(it).contains("@TempDir") }
    if (bad.isNotEmpty()) {
        error("A test using the persistent spike cache must root its project in a per-method"
            + " @TempDir, or the action cache replays the stored record and the code under test never"
            + " runs:\n" + bullets(bad.map { rel(it) }))
    }
}

// ---------------------------------------------------------------------------
// G22 — the IDE clients are wired to jk by string, and nothing else checks the strings.
//
// `clients/intellij` and `clients/vscode` are not modules of either build, so no compiler ever
// looks at them. That is how a rename survived 24 days: a tree-wide sed rewrote a VS Code
// *command id* in package.json, leaving the palette entry pointing at a command extension.js
// never registers — valid JSON naming a handler that does not exist.
//
// Five arms, each reading its expectations from the owning file rather than restating them, and
// each failing loudly when its own scan comes back empty.
// ---------------------------------------------------------------------------

guard("G22", "checkIdeClientWiring") {
    val vscodeManifest = at("clients/vscode/package.json")
    val vscodeSource = at("clients/vscode/extension.js")
    val ideaPluginXml = at("clients/intellij/src/main/resources/META-INF/plugin.xml")
    val ideaBuildScript = at("clients/intellij/build.gradle.kts")
    val ideaJava = at("clients/intellij/src/main/java")
    val wireModel = at("shared/wire/src/main/java/cc/jumpkick/engine/protocol/IdeWireModel.java")
    listOf(vscodeManifest, vscodeSource, ideaPluginXml, ideaBuildScript, wireModel).forEach {
        if (!Files.isRegularFile(it)) error("${rel(it)} is gone, so this guard cannot check the IDE wiring.")
    }
    val cliCommands = filesUnder(at("clients/cli/src/main/java/cc/jumpkick/command"), "Command.java")

    val faults = mutableListOf<String>()
    val manifestText = text(vscodeManifest)
    val extensionText = text(vscodeSource)

    // --- 1. VS Code palette entries and registered handlers are the same set ---
    val declared = Regex(""""command"\s*:\s*"([^"]+)"""").findAll(manifestText).map { it.groupValues[1] }.toSortedSet()
    val registered = Regex("""registerCommand\(\s*"([^"]+)"""").findAll(extensionText).map { it.groupValues[1] }.toSortedSet()
    if (declared.isEmpty()) faults.add("package.json contributes no command at all — arm 1 is blind")
    (declared - registered).forEach { faults.add("package.json contributes command '$it' that extension.js never registers") }
    (registered - declared).forEach { faults.add("extension.js registers command '$it' that package.json never contributes") }

    // --- 2. Every shelled-out verb is a real jk command (names read from the owners) ---
    val verbs = sortedSetOf<String>()
    cliCommands.forEach { f ->
        val raw = text(f)
        Regex("""String name\(\)\s*\{\s*return "([a-z][a-z0-9-]*)";""").findAll(raw)
            .forEach { verbs.add(it.groupValues[1]) }
        Regex("""List<String> aliases\(\)\s*\{[^}]*?return List\.of\(([^)]*)\);""", RegexOption.DOT_MATCHES_ALL)
            .findAll(raw)
            .forEach { m -> Regex(""""([a-z][a-z0-9-]*)"""").findAll(m.groupValues[1]).forEach { verbs.add(it.groupValues[1]) } }
    }
    if (verbs.size < 40) {
        faults.add("verb owner scan found only ${verbs.size} jk commands under"
            + " clients/cli/src/main/java/cc/jumpkick/command — the name()/aliases() shape moved and"
            + " arm 2 is blind")
    }
    val shelled = sortedSetOf<String>()
    Regex("""runJk\(\[\s*"([a-z][a-z0-9-]*)"""").findAll(extensionText).forEach { shelled.add(it.groupValues[1]) }
    val jkCliAction = ideaJava.resolve("cc/jumpkick/idea/JkCliAction.java")
    val jkSyncService = ideaJava.resolve("cc/jumpkick/idea/JkSyncService.java")
    if (Files.isRegularFile(jkCliAction)) {
        Regex("""super\(\s*"[^"]*"\s*,\s*"([a-z][a-z0-9-]*)"""").findAll(text(jkCliAction))
            .forEach { shelled.add(it.groupValues[1]) }
    }
    if (Files.isRegularFile(jkSyncService)) {
        Regex("""JkCliRunner\.run\([^,]+,\s*List\.of\("([a-z][a-z0-9-]*)"""").findAll(text(jkSyncService))
            .forEach { shelled.add(it.groupValues[1]) }
    }
    if (shelled.isEmpty()) faults.add("found no jk verbs invoked by either IDE client — the call shape moved and arm 2 is blind")
    (shelled - verbs).forEach { faults.add("an IDE client shells out to 'jk $it', which is not a name or alias of any :cli command") }

    // --- 3. plugin.xml names classes that exist ---
    val xml = text(ideaPluginXml)
    val ideaClasses = Regex("""(?:class|implementation)="(cc\.jumpkick\.idea\.[A-Za-z0-9_.$]+)"""")
        .findAll(xml).map { it.groupValues[1] }.toSortedSet()
    if (ideaClasses.isEmpty()) faults.add("plugin.xml declares no cc.jumpkick.idea implementation class — arm 3 is blind")
    ideaClasses.forEach { fqcn ->
        // Inner classes (JkCliAction$Sync) live in the outer class's file.
        val outer = fqcn.substringBefore('$')
        val source = ideaJava.resolve(outer.replace('.', '/') + ".java")
        if (!Files.isRegularFile(source)) {
            faults.add("plugin.xml names $fqcn but ${source.fileName} does not exist")
        } else if (fqcn.contains('$')) {
            val inner = fqcn.substringAfter('$')
            if (!text(source).contains("class $inner")) {
                faults.add("plugin.xml names $fqcn but ${source.fileName} declares no class $inner")
            }
        }
    }

    // --- 4. Every wire field the plugin reads is one the engine emits ---
    val jkWireModelFile = ideaJava.resolve("cc/jumpkick/idea/JkWireModel.java")
    if (!Files.isRegularFile(jkWireModelFile)) {
        faults.add("clients/intellij's JkWireModel.java is gone — arm 4 is blind")
    } else {
        val jkWireModel = text(jkWireModelFile)
        val read = sortedSetOf<String>()
        Regex("""str(?:Field|Array)\(\s*(?:body|json)\s*,\s*"([A-Za-z][A-Za-z0-9]*)"""")
            .findAll(jkWireModel).forEach { read.add(it.groupValues[1]) }
        // `workspace` is sniffed as a raw \"key\": literal rather than through the field helpers.
        Regex("""\\"([A-Za-z][A-Za-z0-9]*)\\"\s*:""").findAll(jkWireModel).forEach { read.add(it.groupValues[1]) }
        if (read.isEmpty()) faults.add("JkWireModel reads no recognised wire field — the parse shape moved and arm 4 is blind")
        val emitted = text(wireModel)
        read.forEach { field ->
            if (!emitted.contains("""\"$field\":""")) {
                faults.add("JkWireModel reads wire field '$field' that IdeWireModel.encode() does not emit")
            }
        }
    }

    // --- 5. No untilBuild upper bound on the IntelliJ plugin ---
    Regex("""untilBuild\s*(?:=|\.set\()\s*"([^"]+)"""").find(text(ideaBuildScript))?.let {
        faults.add("clients/intellij pins untilBuild to \"${it.groupValues[1]}\" — a pinned upper bound"
            + " makes the plugin uninstallable the moment JetBrains ships the next build line; unset"
            + " it with `untilBuild = provider { null }`")
    }

    if (faults.isNotEmpty()) {
        error("The IDE clients are not compiled by either build, so their wiring to jk is only ever"
            + " checked here:\n" + bullets(faults.sorted()))
    }
}

// ---------------------------------------------------------------------------
// G14 — a forecast key must hash the same facts as the build key.
//
// `jk explain` re-derives every cache key the build computes. When the two derivations disagree
// the forecast either reports a phantom rebuild (the visible symptom) or blesses a stale artifact
// (the dangerous one). Six such drifts were live at once, and the two tests that claimed to guard
// this compared two hand-typed literals in the TEST FILE against each other instead of reading the
// real bags — which is exactly why they stayed green through all six.
//
// Every `ActionKey.forArtifact` site in the engine must appear in exactly one of three tables, so
// a new key cannot be added without deciding how it is forecast:
//
//   pairs     two bodies, compared. Whole keys legitimately differ (the two sides run at different
//             times over different inputs); the set of `"<prefix>:"` literals may not, because a
//             prefix on one side only means one side hashes a fact the other ignores.
//   shared    ONE body, called by the build and by the forecast. A prefix set cannot see a value
//             drift behind an agreed prefix, and extending the scan to values is not possible
//             since values legitimately differ per module. So the durable answer is one owner, and
//             what the guard checks is that the owner really has both callers. A "shared" key with
//             one caller is an unpaired key wearing the word.
//   unpaired  no forecast key, with the reason — AND whether the forecast emits a STEP for that
//             task at all. Those are different claims, and the difference is where the last defect
//             lived: a site exempted as "not forecast" was true of the key and false of the step,
//             so explain priced a step against a key those builds never compute.
// ---------------------------------------------------------------------------

// A build site and its forecast twin, addressed as `<file>|<key variable>`.
val forArtifactPairs = listOf(
    "package-jar" to ("PlannerPackage.java|pkgKey" to "TaskForecaster.java|pkgKey"))

// One body, both callers. Value: the label, and the `<file>|<literal>` reach points that prove
// each side goes through the owner rather than round it.
...
// forArtifact sites with no forecast twin: (task name, does the forecast emit a step, why there
// is nothing to compare). The boolean is checked against TaskForecaster.
val forArtifactUnpaired = mapOf(
    "PlannerTails.java|key" to Triple("package-sources", false, "explain does not forecast the sources jar at all"),
    "PlannerNative.java|nKey" to Triple("native-image", true, "the forecast probes the task pointer, not a token bag"),
    "PlannerPlugin.java|actionKey" to Triple("plugin-<step>", false, "plugin steps are not forecast (no step, no key)"),
    "ImageWrite.java|imgKey" to Triple("write-image", true,
        "the image tail is an unconditional side-effect step — always RUN, never keyed"),
    "BuildLogicSupport.java|key" to Triple("build-logic", false, "build-logic compile is not forecast"))

// `compileStep` is the one forecast helper that names its step from a parameter; both call sites
// pass a literal. Any OTHER unresolvable step name is a step the scan cannot see.
...
val compileRequestPairs = listOf(
    "compile-test" to ("TestSupport.java|qualifiedTaskId(taskId, outputDir)"
        to "TaskForecaster.java|TaskNames.COMPILE_TEST, testOut)"))

// A CompileRequest with ONE body that both sides call.
...
// Every `CompileRequest.builder()` site in the module and how many times it appears, so a new
// chain has to be declared keyed (above) or unkeyed (here) before the build will run.
...
guard("G14", "checkForecastKeyParity") {
    if (engineMain.size < 330) error("scanned ${engineMain.size} engine main sources; measured against 368.")
    val byName = engineMain.groupBy { it.fileName.toString() }

    fun read(file: String): String {
        val hits = byName[file] ?: error("no $file under server/engine/src/main/java")
        if (hits.size != 1) error("$file is ambiguous: ${hits.map { rel(it) }}")
        return codeOf(hits[0])
    }

    // Java is not parsed here; these walk characters while honouring string and char literals,
    // which is all these shapes need and all a build check should attempt.
    fun balancedFrom(src: String, open: Char, close: Char, at: Int): String {
        var depth = 0
        var i = at
        var inStr = false
        var inChar = false
        ...
        while (i < src.length) {
            ...
            if (esc) esc = false
                ...
                '\'' -> inChar = true
                open -> depth++
                close -> { depth--; if (depth == 0) return src.substring(at + 1, i) }
            }
            i++
        }
    ...
    fun splitTopLevel(args: String): List<String> {
        ...
        for (c in args) {
            if (esc) esc = false
                ...
                '(', '[', '{' -> depth++
                ')', ']', '}' -> depth--
                ',' -> if (depth == 0) { out.add(sb.toString()); sb.setLength(0); continue }
            }
            sb.append(c)
        }
    ...
    fun tokenPrefixes(siteId: String): Set<String> {
        ...
        if (bagAt < 0) {
            error("$siteId no longer keys a `$bagDecl…)` bag. A bag this guard cannot read is a bag"
                + " it cannot compare — give it back that shape, or move the pair onto a shared key"
                + " owner and delete it from the table.")
        }
        val between = src.substring(bagAt, callAt)
        if (between.contains("tokens.add") || between.contains("ActionKey.forArtifact(")) {
            error("$siteId's token bag is amended between its `List.of` and its key, so the literal"
                + " list is not the whole fact set. Same remedy: one readable bag, or one shared owner.")
        }
        return splitTopLevel(balancedFrom(src, '(', ')', bagAt + bagDecl.length - 1))
            .mapIndexed { i, raw ->
                val element = raw.lines().filterNot { it.trim().startsWith("//") }.joinToString("\n").trim()
                Regex("""^"([A-Za-z][A-Za-z0-9_.-]*):""").find(element)?.groupValues?.get(1)
                    ?: error("$siteId token ${i + 1} does not open with a \"prefix:\" literal, so no"
                        + " side can be compared against it: $element")
            }.toSet()
    }
    ...
    val foundSites = engineMain.flatMap { f ->
        siteRegex.findAll(text(f)).map { "${f.fileName}|${it.groupValues[1]}" }
    }.toSortedSet()
    val declaredSites = (forArtifactPairs.flatMap { listOf(it.second.first, it.second.second) }
        + forArtifactShared.keys + forArtifactUnpaired.keys).toSortedSet()
    if (foundSites != declaredSites) {
        faults.add("Every ActionKey.forArtifact site must be declared in the gate, as half of a"
            + " build/forecast pair, as a shared owner both sides call, or as unpaired with the"
            + " reason there is no twin. An undeclared site is a key nobody has decided how to"
            + " forecast.\n    undeclared: ${(foundSites - declaredSites).ifEmpty { "none" }}"
            + "\n    declared but gone: ${(declaredSites - foundSites).ifEmpty { "none" }}")
    } else {
        forArtifactPairs.forEach { (label, sites) ->
            val build = tokenPrefixes(sites.first)
            val forecast = tokenPrefixes(sites.second)
            if (build != forecast) {
                faults.add("$label: the build and the forecast hash different facts, so `jk explain`"
                    + " reports a phantom rebuild (or worse, misses a real one)."
    ...
    // shows up here (the literal is gone) or lands as a new undeclared forArtifact site.
    fun reaches(reach: String, label: String, kind: String) {
        ...
        if (!read(file).contains(literal)) {
            faults.add("$label is declared a shared $kind owner, but $file no longer reaches it"
                + " (`$literal` is gone). A shared owner with one caller is a second body waiting to"
                + " drift: either restore the call, or declare the two sites explicitly so this"
                + " guard compares them instead.")
        }
    }
    forArtifactShared.forEach { (site, spec) ->
        val (label, reachPoints) = spec
        if (reachPoints.size < 2) {
            faults.add("$label ($site) is declared shared but names ${reachPoints.size} caller;"
                + " shared means the build AND the forecast go through one body.")
        }
        reachPoints.forEach { reaches(it, "$label ($site)", "key") }
    }

    // An unpaired key must be honest about the forecast STEP, not just about the key.
    ...
    val unresolved = stepArgs.filterNot {
        it.startsWith("\"") || it.startsWith("TaskNames.") || it in forecastStepIndirections
    }
    if (unresolved.isNotEmpty()) {
        faults.add("TaskForecaster names a step through something this scan cannot resolve:"
            + " $unresolved. A step the scan cannot see is a step an unpaired key can hide behind —"
            + " give it a literal or a TaskNames constant, or declare the indirection in"
            + " forecastStepIndirections and say why it is safe.")
    }
    val taskNames = Regex("""String\s+([A-Z][A-Z_0-9]*)\s*=\s*"([^"]+)"""")
        .findAll(owner(taskNamesOwner))
        .associate { it.groupValues[1] to it.groupValues[2] }
    val forecastSteps = stepArgs.mapNotNull { arg ->
        when {
            ...
            arg.startsWith("TaskNames.") -> taskNames[arg.removePrefix("TaskNames.")]
                ?: error("TaskNames has no constant $arg")
            else -> null
        }
    }.toSortedSet()
    forArtifactUnpaired.forEach { (site, spec) ->
        val (taskName, claimsStep, why) = spec
        val stepped = taskName in forecastSteps
        if (stepped != claimsStep) {
            faults.add(if (stepped) {
                ("$site is exempt as \"$why\", but TaskForecaster DOES emit a `$taskName` step — so"
                    + " explain shows that step priced against no key of its own, or against another"
                    + " step's. Pair the site, move it onto a shared owner, or (if the step is"
                    + " genuinely keyless) set the flag to true and say what decides it instead.")
            } else {
                ("$site claims the forecast emits a `$taskName` step, and it does not. Set the flag to"
                    + " false and state that there is no step, so the next reader is not told a"
                    + " forecast exists that does not.")
            })
        }
    }

    // CompileRequest chains vs what forJavac actually reads.
    val actionKey = read("ActionKey.java")
    val forJavacAt = actionKey.indexOf("public static String forJavac(")
    if (forJavacAt < 0) error("ActionKey.forJavac is gone; the CompileRequest arm has nothing to key off.")
    val forJavacBody = balancedFrom(actionKey, '{', '}', actionKey.indexOf('{', forJavacAt))
    val keyedFields = Regex("""request\.([a-zA-Z][A-Za-z0-9]*)\(\)""")
        .findAll(forJavacBody).map { it.groupValues[1] }.toSortedSet()
    if (keyedFields.isEmpty()) error("forJavac reads no CompileRequest field; the scan has rotted.")

    /** Every setter on the builder chain nearest [siteId]'s marker, followed to its build(). */
    fun keyedSetters(siteId: String): Set<String> {
        ...
        if (markAt < 0 || src.indexOf(marker, markAt + 1) >= 0) {
            error("`$marker` must occur exactly once in $file to address a CompileRequest chain; it"
                + " now occurs ${countLiteral(src, marker)} times. Pick a new marker.")
        }
        val builders = Regex("""CompileRequest\.builder\(\)""").findAll(src).map { it.range.first }.toList()
        val start = builders.minByOrNull { abs(it - markAt) } ?: error("no CompileRequest.builder() in $file")
        val setters = sortedSetOf<String>()
        val setterAt = Regex("""^\.([a-zA-Z][A-Za-z0-9]*)\s*\(""")
        /** Walk a fluent chain from [from], collecting top-level `.name(` setters until its `;`. */
        fun walkChain(s: String, from: Int) {
            var i = from
            var depth = 0
            var inStr = false
            var inChar = false
            ...
            while (i < s.length) {
                ...
                if (esc) esc = false
                    ...
                    ';' -> if (depth == 0) return
                    '.' -> if (depth == 0) {
                        setterAt.find(s.substring(i, minOf(s.length, i + 64)))?.let { setters.add(it.groupValues[1]) }
                    }
                }
                i++
            }
        ...
        walkChain(src, start)
        // Past the primary chain. The fluent chain ends at the first `;`, but a builder held in a
        // local keeps taking setters afterwards — both keyed build sites add the Scala fields in a
        // following `if`, and stopping at the `;` is why an earlier version of this scan could not
        // see the forecast setting none of them. Follow the variable to its build().
        if ("build" !in setters) {
            val assignedTo = Regex("""([A-Za-z_][A-Za-z0-9_]*)\s*=\s*$""")
                .find(src.substring(maxOf(0, start - 200), start))?.groupValues?.get(1)
                ?: error("the CompileRequest chain in $file is not terminated by .build() and is not"
                    + " assigned to a variable, so this scan cannot tell where it ends. Chain it, or assign it.")
            val rest = src.substring(start)
            val buildAt = Regex("""\b${Regex.escape(assignedTo)}\s*\.\s*build\s*\(""").find(rest)
                ?: error("`$assignedTo` in $file never reaches .build(); the continuation scan has no"
                    + " end and would read the rest of the file as part of the chain.")
            // Each `<var>.` statement is itself a fluent chain, so the same walk runs from each one.
            val tail = rest.substring(0, buildAt.range.first)
            Regex("""\b${Regex.escape(assignedTo)}\s*\.""").findAll(tail).map { it.range.last }
                .forEach { walkChain(tail, it) }
        }
        setters.removeAll(setOf("builder", "build"))
        return setters
    }

    val builderCounts = engineMain.mapNotNull { f ->
        val n = countLiteral(text(f), "CompileRequest.builder()")
        if (n == 0) null else f.fileName.toString() to n
    }.toMap()
    if (builderCounts != compileRequestSites) {
        faults.add("Every CompileRequest.builder() chain must be declared in the gate, so a new one"
            + " is classified as keyed (it reaches ActionKey.forJavac and needs a forecast twin) or"
            + " unkeyed (it does not) before it can drift."
            + "\n    found:    ${builderCounts.toSortedMap()}"
            + "\n    declared: ${compileRequestSites.toSortedMap()}")
    } else {
        compileRequestPairs.forEach { (label, sites) ->
            val build = keyedSetters(sites.first).intersect(keyedFields)
            val forecast = keyedSetters(sites.second).intersect(keyedFields)
            if (build != forecast) {
                faults.add("$label: the build's CompileRequest and the forecast's set different fields"
                    + " that ActionKey.forJavac hashes, so the two keys cannot match. forJavac reads:"
                    + " ${keyedFields.toList()}"
                    + "\n    build    ${sites.first}: ${build.sorted()}"
                    + "\n    forecast ${sites.second}: ${forecast.sorted()}"
                    + "\n    only in the build:    ${(build - forecast).sorted()}"
    ...
    compileRequestShared.forEach { (label, spec) ->
        val (ownerId, reachPoints) = spec
        val ownerFile = ownerId.substringBefore('|')
        if (!read(ownerFile).contains(ownerId.substringAfter('|'))) {
            faults.add("$label's shared CompileRequest owner is gone from $ownerFile. Either restore"
                + " it, or declare the two chains as a pair so the field comparison sees them.")
        }
        if (reachPoints.size < 2) {
            faults.add("$label is declared a shared CompileRequest but names ${reachPoints.size}"
                + " caller; shared means the build AND the forecast derive the request from one body.")
        }
        reachPoints.forEach { reaches(it, "$label ($ownerId)", "CompileRequest") }
    }

    if (faults.isNotEmpty()) error("Build/forecast key parity:\n\n" + faults.joinToString("\n\n"))
}
