...
# product data does not uninstall the CLI. Data/cache/state use %LOCALAPPDATA%\jk\...;
# config uses %APPDATA%\jk. See docs/user/install.md.
#
...
function Get-JkCacheRoot {
    if ($env:JK_CACHE_DIR) { return $env:JK_CACHE_DIR }
    if ($env:JK_HOME) { return (Join-Path $env:JK_HOME "cache") }
    $legacyJk = Join-Path $HOME ".jk"
    if (Test-Path -LiteralPath $legacyJk) { return (Join-Path $legacyJk "cache") }
    $local = if ($env:LOCALAPPDATA) { $env:LOCALAPPDATA } else { Join-Path $HOME "AppData\Local" }
    return (Join-Path $local "jk\cache")
}

function Get-JkStoreRoot {
    if ($env:JK_STORE_DIR) { return $env:JK_STORE_DIR }
    if ($env:JK_HOME) { return (Join-Path $env:JK_HOME "store") }
    $local = if ($env:LOCALAPPDATA) { $env:LOCALAPPDATA } else { Join-Path $HOME "AppData\Local" }
    return (Join-Path $local "jk\data\store")
}
...
"@
    ...
    $env:SHELL = "pwsh"
    try {
        Invoke-Jk @("activate", "--yes") 2>&1 | Out-Null
        if ($LASTEXITCODE -ne 0) {
    ...
    if (Test-Command "git") {
        ...
        $cacheTemplates = Join-Path (Get-JkCacheRoot) "templates"
        $tmplKey = ($templatesUrl.ToLowerInvariant() `
            -replace '^https?://', '' `
            -replace '^git@', '' `
            -replace '\.git$', '' `
            -replace '[^a-z0-9._-]', '_')
        $tmplDest = Join-Path $cacheTemplates $tmplKey
        try {
    ...
    # jk-libraries
    $libsDest = Join-Path (Get-JkCacheRoot) "libs.global.toml"
    ...
    try {
        ...
        New-Item -ItemType Directory -Force -Path (Split-Path -Parent $libsDest) | Out-Null
        New-Item -ItemType Directory -Force -Path (Split-Path -Parent $storeLibs) | Out-Null
        $tmpLibs = "$libsDest.tmp"
        Save-Url $libsUrl $tmpLibs
        Move-Item -LiteralPath $tmpLibs -Destination $libsDest -Force
        Copy-Item -LiteralPath $libsDest -Destination $storeLibs -Force
    } catch {
        Remove-Item -LiteralPath "$libsDest.tmp" -Force -ErrorAction SilentlyContinue
    }
