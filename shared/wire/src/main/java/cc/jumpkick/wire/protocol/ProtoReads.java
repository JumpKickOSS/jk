...
public final class ProtoReads {
    ...
    /**
     * As above with the resolved {@link TestSelection}, which explain needs for the same reason
     * {@code jk build} sends it: the selection is an input to every module's {@code run-tests} stamp
     * key, so a request that omits it forecasts against keys no build will ever compute.
     *
     * <p>Concretely, the baseline {@code exclude-tags} live in the <em>root</em> manifest and are
     * layered in by the CLI. {@code effectiveSelection} can only reach a module's own manifest, so
     * an explain that sent no selection stamped every module with an empty tag list, missed the
     * action cache for all of them, and reported "30 modules are dirty · Test ~147 tests" for a
     * build that ran two modules. The suites were priced, too, which is what made a 21 s build
     * forecast at 41 s.
     */
    public static String explainRequest(
            String dir,
        ...
        if (sel.equals(TestSelection.DEFAULT)) return head + "}";
        return head
                + ",\"allSuites\":"
                + sel.allSuites()
                + ",\"suites\":"
                + Jsonl.array(sel.suites())
                + ",\"includeTags\":"
                + Jsonl.array(sel.includeTags())
                + ",\"excludeTags\":"
                + Jsonl.array(sel.excludeTags())
                + ",\"tagsResolved\":"
                + sel.tagsResolved()
                + ",\"gate\":"
                + sel.gate()
    ...
    public static String affectedTestsRequest(
            String dir, TestSelection selection, @Nullable String affectedSince, @Nullable String modules) {
