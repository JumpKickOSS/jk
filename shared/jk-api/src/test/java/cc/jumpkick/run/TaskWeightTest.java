 ...
 * {@code ticks} (internal unit count). The plan denominator sums weights, and a step's own 0→ticks
...
class TaskWeightTest {
    ...
    void denominator_sums_weights_not_scopes() {
        ...
        assertThat(plan.estimatedTotalWeight()).isEqualTo(46); // 40 + 6, not 4 + 6
        plan.run();
        assertThat(plan.snapshot().denominator()).isEqualTo(46);
        assertThat(plan.snapshot().numerator()).isEqualTo(46); // auto-filled on success
    }
    ...
    void overrunning_internal_scope_never_exceeds_the_weight() {
        ...
        assertThat(plan.snapshot().numerator()).isEqualTo(20);
        assertThat(plan.snapshot().denominator()).isEqualTo(20);
    }
    ...
    void reweight_resizes_the_slice_and_the_denominator_mid_run() {
        ...
        assertThat(plan.estimatedTotalWeight()).isEqualTo(50); // 40 + 10, before running
        plan.run();
        // 'a' shrank 40 → 3, so the denominator drops to 3 + 10; both fill fully.
        assertThat(plan.snapshot().denominator()).isEqualTo(13);
        assertThat(plan.snapshot().numerator()).isEqualTo(13);
    }
    ...
    void interpolation_eases_toward_the_weight_and_caps() {
        ...
        BuildPlan plan = BuildPlan.builder("g").build();
        ...
        assertThat(plan.snapshot().numerator()).isEqualTo(50);
        ctx.tick(900_000L); // 90% elapsed → 90
        assertThat(plan.snapshot().numerator()).isEqualTo(90);
