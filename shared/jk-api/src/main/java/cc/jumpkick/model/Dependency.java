...
public record Dependency(
    ...
    /** Defaults {@code pathSource} to null. */
    public Dependency(
            String library,
            String module,
            VersionSelector version,
    ...
    /** Defaults {@code optional} to false and {@code pathSource} to null. */
    public Dependency(
            String library,
            String module,
            VersionSelector version,
    ...
    /** Bare-name git dep; {@code module} is {@link #GIT_PREFIX}{@code <name>}. */
    public static Dependency gitByName(String name, GitSource source) {
        return git(name, GIT_PREFIX + name, source);
    }

    /** Bare-name path dep; {@code module} is {@link #PATH_PREFIX}{@code <name>}, version {@code =path}. */
    ...
    /** Unresolved workspace sibling; {@code WorkspaceMerge} rewrites to the real coord. */
    ...
    /** Version placeholder for a dep whose pin comes from an imported platform BOM. */
    public static final String PLATFORM_MANAGED_VERSION = "platform-managed";

    /** Versionless dep; an imported platform BOM supplies the version at resolve time. */
    ...
    /** True when {@link #version} is the platform-managed placeholder. */
