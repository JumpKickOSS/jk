...
class UnixSocketPathsTest {
    ...
    void a_path_at_the_budget_binds() throws Exception {
        Path dir = Files.createTempDirectory(UnixSocketPaths.shortRoot(), "jk-budget-");
        try {
    ...
    @Test
    void the_short_root_leaves_room_for_a_real_engine_socket_name() {
        ...
        assertThat(UnixSocketPaths.shortRoot().toString().length() + suffix.length())
