...
| See newer versions | `jk outdated` (one row per coordinate an update would move; `--by-module` per module, `--all` for every row); then read `target/jk-outdated-dependencies.md` instead of re-running | `jk_outdated` (`all`; returns the report path as `file`) |
