...
public final class Giter8TemplateIndex {
    ...
    public static Optional<Path> resolveShortName(String shortName, Path... hints) {
        ...
        Path found = findTemplateDir(searchRoots(hints), shortName);
        ...
        if (Giter8ShortNames.find(shortName).isPresent()) {
            ...
            found = findTemplateDir(searchRoots(hints), shortName);
            if (found != null) return Optional.of(found.toAbsolutePath().normalize());
        }
    ...
    static Path findTemplateDir(List<Path> roots, String id) {
        ...
        for (Path root : roots) {
            ...
            Path hit = findNamedUnder(root, id, dirName, 0);
            if (hit != null) return hit;
        }
        return null;
    }

    /** DFS for {@code id.g8} / bare {@code id} template roots, depth-capped. */
    private static Path findNamedUnder(Path dir, String id, String dirName, int depth) {
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(dir)) {
            for (Path child : stream) {
                ...
                String name = child.getFileName().toString();
                if (name.startsWith(".")) continue;
                // Prefer likely containers before a full DFS of huge trees.
                if (name.equals("templates")
                        || name.endsWith(".g8")
                        || name.contains("jk-templates")
                        || name.contains("github.com")
                        || depth == 0) {
                    Path hit = findNamedUnder(child, id, dirName, depth + 1);
                    if (hit != null) return hit;
                }
            }
        } catch (IOException ignored) {
