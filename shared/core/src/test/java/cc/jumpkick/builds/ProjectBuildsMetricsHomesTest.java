...
class ProjectBuildsMetricsHomesTest {
    ...
    @Test
    void same_source_rekey_prefers_the_recently_active_home_over_run_count(@TempDir Path root)
            throws Exception {
        ...
        String path = "/Users/me/jk";
        for (var e : java.util.Map.of(oldHome, "old-id", newHome, "new-id").entrySet()) {
            Files.writeString(
                    e.getKey().resolve(ProjectBuilds.IDENTITY),
                    """
                    id = "%s"
                    coord = "cc.jumpkick:jk"
                    path = "%s"
        ...
        for (int i = 1; i <= 5; i++) {
            ...
            Files.setLastModifiedTime(
                    run, java.nio.file.attribute.FileTime.fromMillis(System.currentTimeMillis() - 86_400_000L));
        }
        Path fresh = newHome.resolve(ProjectBuilds.RUNS).resolve("1");
        Files.createDirectories(fresh);
        Files.setLastModifiedTime(fresh, java.nio.file.attribute.FileTime.fromMillis(System.currentTimeMillis()));
