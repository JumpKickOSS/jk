...
public interface Plugin extends Extension {
     ...
     * cc.jumpkick.plugin.build.BuildExtension} ⇒ {@link cc.jumpkick.plugin.build.Phase#COMPILE},
     * …); a bare {@link cc.jumpkick.plugin.build.BuildPlugin} that only uses {@code register()}
     * declares its phases through its steps' {@code after}/{@code before} instead.
     */
    @Override
    default java.util.Set<cc.jumpkick.plugin.build.Phase> phases() {
        return cc.jumpkick.plugin.build.Capabilities.phasesOf(this);
    }
