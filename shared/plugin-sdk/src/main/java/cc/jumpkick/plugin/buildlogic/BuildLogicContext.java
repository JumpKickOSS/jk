 ...
 * action cache captures and replays. The context used to also hand out the module classes tree;
 * a task that wrote there worked on the first build and silently lost those files on the second,
 * under a reassuring {@code cache hit} label (JK-1614). Everything a task produces goes in
 * {@code outDir} — the engine merges it into classes for you at every anchor except
 * {@code BEFORE_COMPILE}, where it is a generated-source root.
 *
