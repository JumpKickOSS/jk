...
/**
 * A plugin capability: participate in the {@code TEST} phase — wire test-time inputs a
 * framework reads off the classpath. The engine invokes {@link #test} at {@code describe} time to
 * record the plugin's pre-TEST step(s); the declared {@link TaskSpec.Body} runs later with resolved
