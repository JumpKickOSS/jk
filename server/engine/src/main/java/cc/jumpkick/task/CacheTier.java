...
public enum CacheTier {
    ...
    /**
     * Content hashes keyed by absolute path — a single file, which {@link FileHashMemo} keeps
     * capped by entry count in memory and rewrites whole. Retention cannot rank inside it and does
     * not need to; the budget is the backstop for a store that somehow outgrows what the in-memory
     * cap allows, and resetting it costs one rebuild of something wholly derived.
     *
     * <p>No window either: mtime here is a churn clock, rewritten on every flush, so age says
     * nothing about whether the next build wants what is inside.
     */
    HASH_MEMO("hash-memo", Bound.files(null, Bound.resetOverBytes(32L * 1024 * 1024))),
