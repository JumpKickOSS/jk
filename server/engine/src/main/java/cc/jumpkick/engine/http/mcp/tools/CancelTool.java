...
/** {@code cancel} — one jid, or every live job for a checkout. */
public final class CancelTool implements McpTool {
    ...
    public Spec spec() {
        return new Spec(
                ...
                McpSchemas.object(Map.of(
                        "jid",
                        McpSchemas.integer("Job id from build / test / lock / job-start"),
                        "dir",
    ...
    public Map<String, Object> call(McpCall in) {
        ...
        if (dir == null || dir.isBlank()) throw new McpError(-32602, "cancel requires arguments.jid (or dir)");
