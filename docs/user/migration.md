...
behind. A POM that cannot name its project — no `<version>` of its own or its parent's — has no
project to journal under: `jk mvn` prints `run not journaled: <pom> has no <version> and no
<parent><version>` after Maven's own error and writes no row and no report.
...
Tier-2 row — the lock never asks it and names it when an artifact resolves nowhere else; a member
whose `<repository>` id another module of the reactor gave a different URL has it renamed with a
suffix, `nexus-2`, and is a row, since a workspace resolves against one repository set keyed by id),
...
Run 13 (main 7bd7c2299) is the regression the ratchet exists to catch: the `[lint]` tables the
import now writes met three real-tool faults (a SpotBugs launch flag, a Checkstyle config path
resolved against the wrong module, a closure rename race) and the `managed` string form left
jenkins and quarkus with versionless rows no BOM manages, so lock fell to fifteen and build to
nine. Run 14 (main 50257cad1) has those fixes: lock is eighteen — java-design-patterns and
keycloak lock for the first time, through a probe for a version a repository serves while its
metadata lags — while build holds at nine because the lint import still trips on PMD's default
rule set, an empty module's Checkstyle run and a placeholder config, and a managed junit row trips
the Vintage floor check. Each is a ticket; the build count is the next ratchet.
Run 15 (main 86f2a5873) moves it: lock is nineteen and build eleven again — TheAlgorithms-Java is
green through its tests, java-design-patterns, nacos, jenkins and keycloak pass their lint and
compiler-level walls, spring-cloud-alibaba locks under the BOM's junit version — while zipkin loses
its build to a `-J` compiler argument the import now carries into an in-process javac, two
...
| maven-checkstyle-plugin | 14 | each `<execution>` binding `check` is one Checkstyle run — the first the `[lint]` table's own keys, every further one a `[lint.<execution-id>]` entry with its own step and report, an execution Maven skips (`<skip>true</skip>`) a row and no run; in each, `checkstyle` ← `<configLocation>` (a file an ancestor POM's directory holds is named by its path from the module, `../style/checks.xml`; a URL is the key as written and the step fetches it; `${maven.multiModuleProjectDirectory}` is the reactor root — the workspace root, or for a POM imported alone the top of its on-disk parent chain, a `.mvn` directory above the module first — so the root's file is named by its path from the module; a name the module does not hold is a resource of the plugin's `<dependencies>`, which are `checkstyle-classpath`, on the table and every entry; a built-in `sun_checks.xml` / `google_checks.xml`, or a path through another property no POM defines, is written as the POM spelled it plus a row to copy the rule set in — the step is a warning naming it, not a failure, until the file is there), `<suppressionsLocation>` → `checkstyle-suppressions`, `<headerLocation>` → `checkstyle-header`, both resolved the same way, `<propertyExpansion>` → `checkstyle-properties` (its `checkstyle.suppressions.file` and `checkstyle.header.file` lines are those two keys; an absolute path under the module or the reactor root is written by its path from the module, so `${project.build.directory}` is `target`), `<sourceDirectories>` → `sources`, `<excludes>` → `exclude`, `checkstyle-version` ← the plugin's own `<dependencies>` pin, `<includeTestSourceDirectory>` → `sources` + `src/test/java`, `<violationSeverity>warning` → Checkstyle's threshold `warning`, `<failsOnError>false` / `<failOnViolation>false` → `never` (each tool's threshold is `fail-on` when the module's tools agree, else that tool's `<tool>-fail-on`; the default `error` is not written; an entry's `fail-on` is its own) | approximate |
...
| central-publishing-maven-plugin | 11 | `jk publish --central` | exact |
| spotbugs-maven-plugin | 10 | `[lint] spotbugs = true`, `<excludeFilterFile>` → `spotbugs-exclude`, `<effort>` → `spotbugs-effort`, `<threshold>` → `spotbugs-threshold` (`High` → `high`, `Low` / `Exp` → `low`; `Default` is the step's own `medium`), `<maxRank>` → `spotbugs-max-rank`, `<omitVisitors>` / `<visitors>` → `spotbugs-omit-visitors` / `spotbugs-visitors`, `<plugins>` (fb-contrib, find-sec-bugs) → `spotbugs-plugins`, `<includeTests>` → `sources` + `src/test/java` — each parameter read from the `<configuration>` or from the `spotbugs.<name>` user property the POM sets, as the plugin reads it — the plugin version minus its last digit → `spotbugs-version`, SpotBugs's threshold `warning` — `spotbugs:check` fails on any bug at the confidence threshold — or `never` under `<failOnError>false` (written as `fail-on` when the module's tools agree, else `spotbugs-fail-on`) | approximate |
| spotless-maven-plugin | 10 | `jk format` | approximate |
| maven-antrun-plugin | 10 | build logic script → row | manual |
| maven-pmd-plugin | 8 | `[lint] pmd` ← `<rulesets>` (a `/category/…` or `/rulesets/…` path as PMD's built-in, the Maven plugin's own default ruleset — also what a POM naming none runs — as `rulesets/java/maven-pmd-plugin-default.xml`, which jk carries, a `file://` URL as a module file, one spelled through a property nothing defines → `rulesets/java/quickstart.xml` and a row), `<excludeFromFailureFile>` → `pmd-exclude`, `<includeTests>` → `sources` + `src/test/java`, PMD's threshold from its own (`<failOnViolation>false` → `never`, `<failurePriority>` 1 or 2 → `error`, else — Maven's default 5 fails on every finding — `warning`; written as `fail-on` when the module's tools agree, else `pmd-fail-on`), `pmd-version` ← the `pmd-java` the plugin's own `<dependencies>` pin, else the PMD the plugin release bundles, read at import time from the release's own POM in the repositories — its `pmdVersion` property — so every release is covered (a PMD 6 → a row: the step runs PMD 7; a POM no repository serves → a row, and the step's default stands); `<excludeRoots>` / `<excludes>` → row. A lint plugin a module only inherits, with no `<execution>` bound, runs under Maven by hand alone (`mvn pmd:check`): no table on the module, one row at the declaring POM | approximate |
...
| replacer (maven-replacer-plugin) | 1 | beside `protobuf-maven-plugin`, an execution whose `<basedir>` is the plugin's output (`target/generated-sources`) → `[protobuf] replace`, each `<replacement>`'s `<token>` → `<value>` (a `<regex>false</regex>` token quoted); an execution over any other directory — the module's own sources, the generated test sources — → row, since jk rewrites nothing in place; `<skip>true</skip>` executions are left out; `<excludes>`, `<tokenValueMap>`, `<file>`, `<xpath>` → row | approximate |
