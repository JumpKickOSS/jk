...
| maven-checkstyle-plugin | 14 | `[lint] checkstyle` ← `<configLocation>` (a file an ancestor POM's directory holds is named by its path from the module, `../style/checks.xml`; a URL is the key as written and the step fetches it; `${maven.multiModuleProjectDirectory}` is the reactor root — the workspace root, or for a POM imported alone the top of its on-disk parent chain, a `.mvn` directory above the module first — so the root's file is named by its path from the module; a built-in `sun_checks.xml` / `google_checks.xml`, or a path through another property no POM defines, is written as the POM spelled it plus a row to copy the rule set in — the step is a warning naming it, not a failure, until the file is there), `<suppressionsLocation>` → `checkstyle-suppressions`, resolved the same way, `<excludes>` → `exclude`, `checkstyle-version` ← the plugin's own `<dependencies>` pin, `<includeTestSourceDirectory>` → `sources` + `src/test/java`, `<violationSeverity>warning` → Checkstyle's threshold `warning`, `<failsOnError>false` / `<failOnViolation>false` → `never` (each tool's threshold is `fail-on` when the module's tools agree, else that tool's `<tool>-fail-on`; the default `error` is not written) | approximate |
