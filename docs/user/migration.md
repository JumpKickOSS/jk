...
| maven-checkstyle-plugin | 14 | `[lint] checkstyle` ← `<configLocation>` (a file an ancestor POM's directory holds is named by its path from the module, `../style/checks.xml`; a URL is the key as written and the step fetches it; a built-in `sun_checks.xml` / `google_checks.xml`, or a path through a property only the Maven launcher sets, is written as the POM spelled it plus a row to copy the rule set in — the step is a warning naming it, not a failure, until the file is there), `<excludes>` → `exclude`, `checkstyle-version` ← the plugin's own `<dependencies>` pin, `<includeTestSourceDirectory>` → `sources` + `src/test/java`, `<violationSeverity>warning` → Checkstyle's threshold `warning`, `<failsOnError>false` / `<failOnViolation>false` → `never` (each tool's threshold is `fail-on` when the module's tools agree, else that tool's `<tool>-fail-on`; the default `error` is not written) | approximate |
