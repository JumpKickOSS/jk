...
| spotbugs-maven-plugin | 10 | `[lint] spotbugs = true`, `<excludeFilterFile>` → `spotbugs-exclude`, `<effort>` → `spotbugs-effort`, `<threshold>` → `spotbugs-threshold` (`High` → `high`, `Low` / `Exp` → `low`; `Default` is the step's own `medium`), `<includeTests>` → `sources` + `src/test/java`, the plugin version minus its last digit → `spotbugs-version`, SpotBugs's threshold `warning` — `spotbugs:check` fails on any bug at the confidence threshold — or `never` under `<failOnError>false` (written as `fail-on` when the module's tools agree, else `spotbugs-fail-on`); `<plugins>` (fb-contrib, find-sec-bugs) → row | approximate |
