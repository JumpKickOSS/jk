...
| **Store** (`JK_STORE_DIR`) | Maven-layout `repos/` + `.jk` memos (deps, workers). Maven local repo is the primary jar store when `m2integration` is on (default). | `jk storage nuke` (does **not** delete `~/.m2`) |
