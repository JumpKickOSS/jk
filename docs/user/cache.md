...
`jk-lock.toml` always rebuilds. Caching only speeds up CAS downloads and action hits.
...
| **Store** (`JK_STORE_DIR`) | Artifact CAS + `repos/` mirrors (deps, workers) | `jk storage nuke` |
...
The store’s 6 GiB default is **display-only**. `jk storage clean` never evicts reachable
store blobs; it only reclaims garbage (`.put-` temps, expired run logs, unreferenced CAS).
