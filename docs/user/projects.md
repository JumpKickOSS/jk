...
| `[m2] integration` | Use the Maven local repository as the primary third-party jar store (default **true**). `false` hosts those jars only under `JK_STORE_DIR/repos/<name>/`. First-party workers always stay in `repos/local`. Machine override: `JK_M2_INTEGRATION=false` or user-config `[m2] integration = false`. |
| `[m2] install` | Write `jk install` artifacts into the Maven local repository (default **true**). Independent of `integration`: `[m2] install = false` keeps `jk install` under `repos/local` even when third-party jars still come from `~/.m2`. Machine override: `JK_M2_INSTALL=false` or user-config `[m2] install = false`. |
