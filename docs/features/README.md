# Moved

User-facing topics that used to live as “feature PRDs” are now product pages:

- [Packaging](../user/packaging.md)
- [Templates](../user/templates.md)
- [Platforms / catalogs](../user/platforms.md)
- [Build logic](../user/build-logic.md)
- [Dynamic surface](../user/dynamic-surface.md)

Design records and PRDs: KanArtist `projects/jk/docs/prds/`.
Examples: [../user/examples/](../user/examples/).
