...
jk new --template quarkus/hello my-api # Giter8 template (same single-module shape)
```
...
application. Measured on the `jk new -t spring-boot/hello` scaffold: **771 ms cold, 325 ms cached**.
...
Grails 8 (Apache, Spring Boot 4) on the Groovy lane — `jk new -t grails/hello` scaffolds a
...
jk new --template quarkus/hello x  # Giter8 template (or local path)
...
The wrapper is a **bootstrapper, not a pin**: it runs the newest installed jk that satisfies
the lock's optional `jk-min` floor, otherwise fetches the latest release (verified against the
release's `SHA256SUMS`). Team version alignment belongs to the installer / CI images, never to
a lock row. `jk update` re-resolves dependencies; `jk self update` updates the tool — two
deliberate steps.
