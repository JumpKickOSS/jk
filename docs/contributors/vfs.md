...
(JK-1031 / `PathUtil` javadoc). macOS sits between. The VFS exists so a job
...
```toml
# ~/.config/jk/config.toml  — machine-scoped, not project-overridable
...
| **`0`** | VFS **off**: every query live-walks. Debug / A-B. Not “unlimited.” |
