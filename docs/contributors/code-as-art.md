...
| G33 | `gradle/libs.versions.toml` and `jk-lock.toml` disagreeing on a shared module version | ban | retired: Gradle is bootstrap-only and jk is the gate; the catalog-to-lock agreement is G106 `gradle-bootstrap-parity`'s |
...
| G89 | a literal `group:artifact:version` coordinate in a Gradle script — every pin lives in gradle/libs.versions.toml, which the lockfiles and verification metadata are written from | text, `**/*.gradle.kts`, code only | retired: Gradle is bootstrap-only and jk is the gate; the lock is the one version source, and G106 holds the catalog to it |
