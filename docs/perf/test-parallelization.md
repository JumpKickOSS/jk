...
| Layer | Behavior | Code |
|-------|----------|------|
| Cross-module test gate | Serial unless `Session.parallelTests` | `BuildBuildPlans` test gate; CLI `--parallel-tests` |
...
- Engine: `JUnitLauncher`, `BuildBuildPlans` test gate, `HeapPlan.requestedJvms`  
