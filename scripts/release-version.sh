...
# says which check failed and exits 1. The tag or dispatch input is the request; the source is
# the answer — a run that publishes a tree stamped with one version under another's name is
...
if [[ -z "$REQUESTED" ]]; then
  echo "release-version: no version requested (tag vX.Y.Z or the dispatch input)" >&2
  exit 1
fi
