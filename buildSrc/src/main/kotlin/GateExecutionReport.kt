...
/**
 * Says which test tiers this build actually ran, and reads their counts out of `TEST-*.xml` rather
 * than believing the task outcome.
 *
 * Why it exists (JK-1018): a `checkAll` run reported nine failures when there were ten, because
 * `:cli:test` was `UP-TO-DATE` and never executed — a red tier was invisible in the gate that was
 * supposed to be the merge bar. A later run of the same gate finished in three seconds with 899 of
 * 928 tasks up-to-date and results served from the build cache, and printed `BUILD SUCCESSFUL`.
 * Both are correct Gradle behaviour and both are useless as evidence, because the thing a human
 * reads off a gate is "the tests passed", and what the gate said was "nothing needed doing".
 *
 * A cached pass is a fine answer to "is this input already known good". It is not an answer to
 * "did this tier run", and the two are indistinguishable in the default output.
 *
 * Mechanics: a [BuildService] listening to task completion, which is the outcome the build itself
 * saw — not a re-derivation. `UP-TO-DATE` and `FROM-CACHE` both mean the task body did not run, so
 * both are reported as not executed. Counts come from the XML because a tier can execute, report
 * `BUILD SUCCESSFUL` for the task, and still have been filtered down to zero tests.
 */
abstract class GateExecutionReport :
    ...
    private enum class Ran { EXECUTED, NOT_EXECUTED, SKIPPED, FAILED }
    ...
    override fun onFinish(event: FinishEvent) {
        ...
        outcomes[path] = when (val result = event.result) {
            is TaskFailureResult -> Ran.FAILED
            is TaskSkippedResult -> Ran.SKIPPED
            is TaskSuccessResult ->
                    if (result.isUpToDate || result.isFromCache) Ran.NOT_EXECUTED else Ran.EXECUTED
    ...
    override fun close() {
        ...
        outcomes.toSortedMap().forEach { (path, ran) ->
            ...
            val tally = if (c == null) "no TEST-*.xml found"
            ...
            when (ran) {
                ...
                Ran.SKIPPED -> { quiet++; lines.add("  skipped      $path — did not run") }
                Ran.NOT_EXECUTED -> {
                    quiet++
        ...
        if (quiet > 0) {
            println("  ^ $quiet tier(s) did not execute in this build. Their counts above are"
    ...
    /**
     * Sum every `TEST-*.xml`, keyed `<module dir leaf>/<tier>`. Keyed per module, not per tier: a
     * single bucket per tier sums every module's results and then reports that total against each
     * task, which reads as every tier having run every test in the tree.
     */
    private fun resultsByModuleTier(): Map<String, Tally> {
        ...
        root.walkTopDown()
                ...
                .forEach { tierDir ->
                    ...
                    tierDir.listFiles { f: File -> f.name.startsWith("TEST-") && f.extension == "xml" }
 ...
 *
 * Split out so the one assumption in this reporter is testable: that a module's directory leaf
 * matches its Gradle project name. That holds across this tree — `:core` is `shared/core`, `:cli`
 * is `clients/cli`, `:toolchain-jdk` is `shared/toolchain-jdk` — and where it ever stops holding
 * the lookup misses and the report says "no TEST-*.xml found" rather than attributing another
 * module's numbers, which is the failure mode that matters.
 */
