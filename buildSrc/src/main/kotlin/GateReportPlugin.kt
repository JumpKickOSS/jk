 ...
 *
 * A plugin rather than a few lines in the root script because `BuildEventsListenerRegistry` is only
 * available by injection, and doing it here keeps the root build file to a one-line `apply`.
 */
abstract class GateReportPlugin @Inject constructor(private val events: BuildEventsListenerRegistry) :
        Plugin<Project> {
    ...
    override fun apply(target: Project) {
        ...
        val service = target.gradle.sharedServices.registerIfAbsent(
                "gateExecutionReport", GateExecutionReport::class.java) {
