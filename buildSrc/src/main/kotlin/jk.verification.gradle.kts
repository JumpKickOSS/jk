...
// ---------------------------------------------------------------------------
val checkNoHandBuiltJavaBinary = registerGuard("checkNoHandBuiltJavaBinary") {
    group = "verification"
    ...
    doLast {
        ...
        val hits = mainJava.files.sorted().filter { it != ownerFile }.flatMap { f ->
            ...
            val code = GuardScan.guardText(f.readText())
...
val checkFileSizeCaps = registerGuard("checkFileSizeCaps") {
    group = "verification"
    ...
    // per LANGUAGE, and `.java` is `.java` — so this task walks `src/main`, `src/test`, and
    // `src/fixtures`. A rule stated for an extension is enforced for every directory that holds
    // that extension; the doc/guard parity arm below compares extensions and is blind to
    // directory scope.
    //
    // Splitting a suite can duplicate a fixture; that argues for the exception band, not for a
    // second number. The band already exists: `size-baseline.txt` plus a stated invariant,
...
val checkNoFqcn = registerGuard("checkNoFqcn") {
    group = "verification"
    ...
    doLast {
        ...
        sources.files.sorted().forEach { f ->
            val rel = f.relativeTo(treeRoot).invariantSeparatorsPath
            val n = GuardScan.countFqcns(f.readText())
            present.add(rel)
            val at = listed[rel]
            when {
...
val checkSingleXmlParserOwner = registerGuard("checkSingleXmlParserOwner") {
    group = "verification"
    ...
    doLast {
        ...
        val ownerCode = GuardScan.guardText(owner.asFile.readText())
...
// ---------------------------------------------------------------------------
val checkWireProtocolPrefixPairs = registerGuard("checkWireProtocolPrefixPairs") {
    group = "verification"
...
val checkSingleTruthSet = registerGuard("checkSingleTruthSet") {
    group = "verification"
    ...
    doLast {
        ...
        mainJava.files.sorted().forEach { f ->
            val n = GuardScan.countIn(GuardScan.guardText(f.readText()), handRolled)
            if (n > 0) hits[f.relativeTo(treeRoot).invariantSeparatorsPath] = n
        }
        val (grew, unlisted, loose) = GuardScan.ratchetVerdict(hits, allowed, here)
...
// ---------------------------------------------------------------------------
val checkOneDigestSurface = registerGuard("checkOneDigestSurface") {
    group = "verification"
    ...
    doLast {
        ...
        val ownAlgorithm = Regex("""newSha256\(\)\{returnnewDigest\("([^"]+)"\);}""")
                .find(GuardScan.guardText(ownerFile.readText()))
                ?.groupValues
                ?.get(1)
        ...
        mainJava.files.sorted().forEach { f ->
            ...
            val code = GuardScan.guardText(f.readText())
            val rel = f.relativeTo(treeRoot).invariantSeparatorsPath
            GuardScan.countIn(code, lookup).let {
                if (it > 0) hits.add("  $rel: $it x MessageDigest.getInstance(..)")
            }
            GuardScan.countIn(code, ownAlgorithmByName).let {
...
// ---------------------------------------------------------------------------
val checkNoHandRolledHex = registerGuard("checkNoHandRolledHex") {
    group = "verification"
    ...
    doLast {
        ...
        mainJava.files.sorted().forEach { f ->
            val src = f.readText()
            val rel = f.relativeTo(treeRoot).invariantSeparatorsPath
            GuardScan.countIn(GuardScan.guardText(src), hexLoop).let {
            ...
            val withImports = GuardScan.squashBetweenLiterals(GuardScan.blankNonCode(src, blankStrings = false))
            GuardScan.countIn(withImports, hexFormat).let {
...
val checkNoBareTaskName = registerGuard("checkNoBareTaskName") {
    group = "verification"
    ...
    doLast {
        ...
        mainJava.files.sorted().forEach { f ->
            ...
            named.forEach { (value, constant) ->
                val n = GuardScan.countIn(code, Regex(Regex.escape("\"$value\"")))
...
// ---------------------------------------------------------------------------
val checkNoBareManifestName = registerGuard("checkNoBareManifestName") {
    group = "verification"
...
// ---------------------------------------------------------------------------
val checkNoBareExitCode = registerGuard("checkNoBareExitCode") {
    group = "verification"
    ...
    doLast {
        ...
        mainJava.files.sorted().forEach { f ->
            ...
            val code = GuardScan.blankNonCode(f.readText()).replace(Regex("\\s+"), "")
            val rel = f.relativeTo(treeRoot).invariantSeparatorsPath
...
// ---------------------------------------------------------------------------
val checkSingleArchiveInstant = registerGuard("checkSingleArchiveInstant") {
    group = "verification"
    ...
    doLast {
        ...
        mainJava.files.sorted().forEach { f ->
            val code = GuardScan.guardText(f.readText())
            val rel = f.relativeTo(treeRoot).invariantSeparatorsPath
            val byTime = GuardScan.countIn(code, Regex("""\.setTime\("""))
            if (byTime > 0) hits.add("  $rel: $byTime x setTime(  ->  DeterministicZip.entry")
            if (f == ownerFile) return@forEach
            val byLocal = GuardScan.countIn(code, Regex("""\.setTimeLocal\("""))
...
val checkNoBareTierName = registerGuard("checkNoBareTierName") {
    group = "verification"
        ...
        mainJava.files.sorted().forEach { f ->
            ...
            if (f != osOwnerFile) {
                properties.forEach { prop ->
                    ...
                    val n = GuardScan.countIn(code, pattern)
                    if (n > 0) unownedOsReads.add("  $rel: $n x System.getProperty(\"$prop\")")
                }
            }
            if (f !in separatorOwners) {
                val n = separators.sumOf { GuardScan.countIn(code, it) }
...
// ---------------------------------------------------------------------------
val checkToolchainEnvFromRequest = registerGuard("checkToolchainEnvFromRequest") {
    group = "verification"
...
// ---------------------------------------------------------------------------
val checkArchiveStreamOwner = registerGuard("checkArchiveStreamOwner") {
    group = "verification"
...
// ---------------------------------------------------------------------------
val checkCheapestRejectionFirst = registerGuard("checkCheapestRejectionFirst") {
    group = "verification"
...
// ---------------------------------------------------------------------------
val checkRunnableOwner = registerGuard("checkRunnableOwner") {
    group = "verification"
...
// trip it.
// ---------------------------------------------------------------------------
val checkTreeCopyOwner = registerGuard("checkTreeCopyOwner") {
    group = "verification"
...
// ---------------------------------------------------------------------------
val checkBlindWalkRatchet = registerGuard("checkBlindWalkRatchet") {
    group = "verification"
...
val checkSingleCentralAddress = registerGuard("checkSingleCentralAddress") {
    group = "verification"
    ...
    doLast {
        ...
        mainJava.files.sorted().forEach { f ->
            ...
            val code = GuardScan.guardText(f.readText())
            val aliased = GuardScan.countIn(code, Regex(Regex.escape(alias)))
            if (aliased > 0) {
            ...
            named.forEach { (value, constant) ->
                ...
                val n = GuardScan.countIn(code, Regex("""(?<!"cc",)""" + Regex.escape("\"$value\"")))
                if (n > 0) {
...
// ---------------------------------------------------------------------------
val checkNoBareWireType = registerGuard("checkNoBareWireType") {
    group = "verification"
    description = "Fail the build on a wire message type typed as a literal (use EngineProtocol)"
    val mainJava = fileTree(layout.projectDirectory.dir("src/main/java")) { include("**/*.java") }
    inputs.files(mainJava).withPropertyName("mainJava")
    val owner = rootProject.layout.projectDirectory.file(
            "shared/wire/src/main/java/cc/jumpkick/wire/protocol/EngineProtocol.java")
    ...
    doLast {
        ...
        val named = Regex("""public static final String (\w+) = "([^"]*)";""")
                ...
                .toMap()
        if (named.isEmpty()) {
            throw GradleException("cc.jumpkick.wire.protocol.EngineProtocol no longer declares any"
        ...
        if (hits.isNotEmpty()) {
            throw GradleException("A wire message type typed as a literal is a producer/consumer"
                    ...
                    + hits.sorted().joinToString("\n")
                    + "\n  Reference cc.jumpkick.wire.protocol.EngineProtocol instead. A string"
...
val checkNoBareTargetDir = registerGuard("checkNoBareTargetDir") {
    group = "verification"
    ...
    doLast {
        ...
        mainJava.files.sorted().forEach { f ->
            val rel = f.relativeTo(treeRoot).invariantSeparatorsPath
            if (f == owner.asFile || rel in exempt) return@forEach
            val n = GuardScan.countIn(GuardScan.guardText(f.readText()), Regex(Regex.escape("\"$value\"")))
...
pluginManager.withPlugin("maven-publish") {
    val checkPublishedPomCoordinates = registerGuard("checkPublishedPomCoordinates") {
        group = "verification"
...
val checkOneJsonCodec = registerGuard("checkOneJsonCodec") {
    group = "verification"
...
val checkNoOrphanTestTags = registerGuard("checkNoOrphanTestTags") {
    group = "verification"
    ...
    doLast {
        ...
        testJava.files.sorted().forEach { f ->
            ...
            val code = GuardScan.blankNonCode(raw, blankStrings = false)
...
val checkTestPathsFromCheckoutRoot = registerGuard("checkTestPathsFromCheckoutRoot") {
    group = "verification"
    ...
    doLast {
        ...
        (testJava.files + fixtureJava.files).sorted().forEach { f ->
            ...
            val code = GuardScan.guardText(f.readText())
            val rel = f.relativeTo(treeRoot).invariantSeparatorsPath
            val walks = GuardScan.countIn(code, Regex(Regex.escape("getProtectionDomain")))
            if (walks > 0) {
            ...
            GuardScan.blankNonCode(f.readText(), blankStrings = false).lines().forEachIndexed { i, line ->
...
val checkManifestDepParity = registerGuard("checkManifestDepParity") {
    group = "verification"
    ...
    doLast {
        ...
        var edges = 0
        GuardScan.blankNonCode(scriptFile.readText(), blankStrings = false).lines().forEach { line ->
...
val checkOneRecursiveDelete = registerGuard("checkOneRecursiveDelete") {
    group = "verification"
    ...
    doLast {
        ...
        mainJava.files.sorted().forEach { f ->
            ...
            val lines = GuardScan.blankNonCode(f.readText(), blankStrings = false).lines()
...
val checkJdkRemovalConfined = registerGuard("checkJdkRemovalConfined") {
    group = "verification"
...
val checkCaseConversionLocale = registerGuard("checkCaseConversionLocale") {
    group = "verification"
    ...
    doLast {
        ...
        val hits = mutableListOf<String>()
        mainJava.files.sorted().forEach { f ->
            val n = GuardScan.countIn(GuardScan.guardText(f.readText()), bare)
...
val checkNoGluedInlineTag = registerGuard("checkNoGluedInlineTag") {
    group = "verification"
    ...
    doLast {
        ...
        allJava.files.sorted().forEach { f ->
            val n = GuardScan.countIn(f.readText(), glued)
            if (n > 0) hits.add("  ${f.relativeTo(treeRoot).invariantSeparatorsPath}: $n")
        }
        if (hits.isNotEmpty()) {
