    ...
    val all: List<GuardSpec> =
        listOf(
            ...
            spec(
                92,
                "json-concat",
                "a JSON object line built by string concatenation — a literal opening `{\"` or a `+ \",\\\"field\\\"` fragment — in main code; measured here, held by G93",
                "text, `**/src/main/java`, comments blanked; measured only (the owner is the tree)",
                GuardHome.SELF_HOSTED,
                tableTask = "`json-concat` (jk-guards.toml, `text`)",
                inFastGate = false,
                gradleLetter = false,
                attach = emptySet(),
                ruleId = "json-concat",
            ...
            spec(
                93,
                "json-concat-ratchet",
                "a file's count of hand-built JSON fragments growing past its baseline — the count only falls, until the pattern is a ban",
                "metric, `matches:json-concat` per file, baselined and tightened on every build",
                GuardHome.SELF_HOSTED,
                tableTask = "`json-concat-ratchet` (jk-guards.toml, `metric`)",
                inFastGate = false,
                gradleLetter = false,
                attach = emptySet(),
            ...
            spec(
                ...
                "a schema field compared to an integer literal (`schema >= 3`) instead of to its `SCHEMA` constant — G85 pins the constant, this pins every comparison against it",
            ...
            spec(
                ...
                "a git verb after `-C <dir>` that is not a known read — the write searches upward and lands in whatever repository encloses the path, not the one meant",
                "text, no match tree-wide",
                GuardHome.SELF_HOSTED,
            ...
            spec(
                96,
                "engine-log-owner",
                "a write to System.out / System.err, printStackTrace() or dumpStack() in server or shared main code outside `cc.jumpkick.host.Log` — a print has no level, misses the engine log's size cap and skips its redaction",
                "forbid, `@jdk-system-out`, scope `server/*` and `shared/*`",
                GuardHome.SELF_HOSTED,
                tableTask = "`engine-log-owner` (jk-guards.toml, `forbid`)",
                inFastGate = false,
                gradleLetter = false,
                attach = emptySet(),
                ruleId = "engine-log-owner",
            ),
            spec(
                97,
                "swallowed-broad-catch",
                "a catch of Exception, Throwable or RuntimeException whose body is only a comment, in main code; measured here, held by G98",
                "text, `**/src/main/java`, comments blanked; measured only (the owner is the tree)",
                GuardHome.SELF_HOSTED,
                tableTask = "`swallowed-broad-catch` (jk-guards.toml, `text`)",
                inFastGate = false,
                gradleLetter = false,
                attach = emptySet(),
                ruleId = "swallowed-broad-catch",
            ),
            spec(
                98,
                "swallowed-broad-catch-ratchet",
                "a file's count of comment-only broad catches growing past its baseline — zero since every one got a debug line carrying the exception",
                "metric, `matches:swallowed-broad-catch` per file, baselined and tightened on every build",
                GuardHome.SELF_HOSTED,
                tableTask = "`swallowed-broad-catch-ratchet` (jk-guards.toml, `metric`)",
                inFastGate = false,
                gradleLetter = false,
                attach = emptySet(),
