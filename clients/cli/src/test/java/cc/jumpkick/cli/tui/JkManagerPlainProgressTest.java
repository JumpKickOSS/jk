...
class JkManagerPlainProgressTest {
    ...
    void plain_progress_emits_stage_changes_not_percent_ticks() {
        ...
        SessionContext.runWhere(Session.defaults().withConfig(noAnsi), () -> {
            ...
            assertThat(out.split("Examining source files :: ", -1).length - 1).isEqualTo(1);
        });
    }
    ...
    void plain_progress_emits_phase_and_built_immediately() {
        ...
        SessionContext.runWhere(Session.defaults().withConfig(noAnsi), () -> {
            ...
            assertThat(out.split("- compiling 12 sources", -1).length - 1).isEqualTo(1);
    ...
    void plain_progress_heartbeats_long_running_stages_every_30s() {
        ...
        SessionContext.runWhere(Session.defaults().withConfig(noAnsi), () -> {
            ...
            try {
                ...
                assertThat(out.split("- running 860 tests", -1).length - 1).isEqualTo(1);
                assertThat(out.split("- running 759 tests", -1).length - 1).isEqualTo(1);
            } finally {
                cm.finishBuildPlanSuccess("Build successful, built 1 module - took 1s", List.of());
            }
        });
    ...
    void plain_process_output_surfaces_on_step_failure() {
        ...
        SessionContext.runWhere(Session.defaults().withConfig(noAnsi), () -> {
            ...
            assertThat(again.indexOf("native-image: Error")).isEqualTo(again.lastIndexOf("native-image: Error"));
        });
    }
