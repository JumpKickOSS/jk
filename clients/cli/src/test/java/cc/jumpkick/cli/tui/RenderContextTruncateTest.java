...
/** Visible-width truncation and row column budget — {@link RenderContext}'s clipping half. */
class RenderContextTruncateTest {
    ...
    void truncate_visible_overflow_by_one_still_shows_ellipsis() {
        ...
        for (int n = 2; n <= 40; n++) {
            String cut = RenderContext.truncateVisible("x".repeat(n + 1), n);
    ...
    void row_column_budget_leaves_last_column_free() {
        assertThat(RenderContext.rowColumnBudget(80)).isEqualTo(79);
        assertThat(RenderContext.rowColumnBudget(2)).isEqualTo(1);
        assertThat(RenderContext.rowColumnBudget(1)).isEqualTo(1);
        assertThat(RenderContext.rowColumnBudget(0)).isEqualTo(1);
    }
    ...
    void truncate_visible_drops_control_characters_instead_of_emitting_them() {
        ...
        assertThat(RenderContext.truncateVisible("ab\tcd\re", 10)).isEqualTo("abcde");
        assertThat(RenderContext.truncateVisible("a\bb", 2)).isEqualTo("ab");
        assertThat(RenderContext.visibleWidth(RenderContext.truncateVisible("a\tb\tc", 3)))
                .isEqualTo(3);
    }
    ...
    void truncate_visible_zero_width_tail_is_not_an_ellipsis_reserve() {
        ...
        assertThat(RenderContext.truncateVisible(cafe, 4)).isEqualTo(cafe);
        String sun = "\u2600\uFE0F"; // emoji + VS16 at its exact width
        assertThat(RenderContext.truncateVisible(sun, 1)).isEqualTo(sun);
        assertThat(RenderContext.truncateVisible("abc\t", 3)).isEqualTo("abc"); // dropped-control tail
        // A tail that still costs columns keeps the reserve.
        assertThat(TestAnsi.strip(RenderContext.truncateVisible("cafe\u0301s", 4)))
                .isEqualTo("caf…");
    }
    ...
    void truncate_visible_one_column_keeps_a_fitting_char() {
        ...
        assertThat(RenderContext.truncateVisible("", 1)).isEmpty();
        assertThat(RenderContext.truncateVisible("a", 1)).isEqualTo("a");
        assertThat(TestAnsi.strip(RenderContext.truncateVisible("ab", 1))).isEqualTo("…");
    }
}
