...
/** Two sequential modules feed one aggregate view: counts sum, phase pills merge. */
class AggregateModuleListenerTest {

    private static final long PF = AggregateContext.PREFLIGHT_UNITS;

    @Test
    void modules_accumulate_into_one_bar_and_phase_chain() {
        ...
        // Phase chain: only running Test (Compile succeeded and was removed).
    ...
    @Test
    void concurrent_modules_sum_into_the_bar_and_phase_tree() {
        var buf = new ByteArrayOutputStream();
        CommandManager view =
                CommandManager.pipeline(new PrintStream(buf, true, StandardCharsets.UTF_8), "Building", false);
        var agg = new AggregateContext(view);
        agg.calibrate(20); // two modules, slice 10 each → den = PF + 20
        ...
        // Aggregate = PF + 5 + 5 of PF + 20 → 110/120 ≈ 92%
        String all = String.join(
                "\n",
                view.renderPipelineLines(120, 0).stream()
                        .map(AggregateModuleListenerTest::strip)
